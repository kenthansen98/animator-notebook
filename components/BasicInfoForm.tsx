...
const BasicInfoForm: React.FC<Props> = ({ toggleAdd, notebookId }) => {
    ...
    const [jyList, setJyList] = useState<
        { name: string; age: string; notes: NewNote[] }[]
    >([]);
        ...
        <form onSubmit={onSubmit}>
            {jyList.map((jy, i) => (
                <div key={i}>
                    ...
                    <button
                        onClick={() =>
                            setJyList(jyList.filter((_jy, idx) => idx !== i))
                        }
                    >
                        x
                    </button>
            ...
            <div>
                ...
                <input
                    ...
                    placeholder="Name"
                    value={jyName}
                    onChange={(e) => setJyName(e.target.value)}
                />
                <input
                    type="number"
                    min="10"
                    ...
                    placeholder="Age"
                    value={jyAge}
                    onChange={(e) => setJyAge(e.target.value)}
                />
            </div>
            {jyNotes.map((note, i) => (
                <div key={i}>
            ...
            <input
                disabled={!jyName || !jyAge}
                type="button"
                value="Add"
                onClick={onAddJy}
            />
            <input
                disabled={jyList.length === 0}
                type="submit"
                value="Confirm"
            />
        </form>
